...
@router.post("/cities/{city_slug}/onboarding")
async def onboard_city(
    city_slug: str,
    name: Optional[str] = Form(None),
    state: Optional[str] = Form(None),
    plano_governo: UploadFile = File(None),
    despesas: UploadFile = File(None),
    receitas: UploadFile = File(None)
):
    """
    Onboard a city with data files.
    - Creates city if not exists (requires name and state in this case).
    - Ingests Expenses (CSV).
    - Ingests Revenues (CSV).
    - Uploads Government Plan (PDF).
    """
    supabase = get_supabase_client()
    ingestion = DataIngestionService()
    
    # 1. Ensure City Exists
    city_res = supabase.table("cities").select("*").eq("slug", city_slug).execute()
    
    if not city_res.data:
        # Create City
        if not name or not state:
            raise HTTPException(status_code=400, detail="City not found. Provide 'name' and 'state' to create it.")
            
        print(f"🏙️ Creating new city: {name} ({state})")
        new_city = {
            "name": name,
            "state": state.upper(),
            "slug": city_slug
    ...
    # Expenses
    if despesas:
        print(f"📄 Processing Expenses CSV: {despesas.filename}")
        content = await despesas.read()
        res = await ingestion.ingest_expenses(city_slug, content)
        results["expenses"] = res
        
    # Revenues
    if receitas:
        print(f"💰 Processing Revenues CSV: {receitas.filename}")
        content = await receitas.read()
        res = await ingestion.ingest_revenues(city_slug, content)
        results["revenues"] = res

    # Government Plan
    if plano_governo:
        print(f"📜 Uploading Government Plan: {plano_governo.filename}")
        res = await ingestion.upload_government_plan(city_slug, plano_governo)
        results["plano_governo"] = res
