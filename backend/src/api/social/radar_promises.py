...
async def refresh_radar(campaign_id: UUID, politico_id: str):
    ...
    if pol.data and pol.data[0].get("city_id"):
        ...
        if city.data:
            municipio_slug = city.data[0].get("slug")
    
    if not municipio_slug:
        raise HTTPException(status_code=400, detail="Cidade não encontrada para este político. O slug da cidade é obrigatório para o Radar.")
...
@router.post("/{campaign_id}/radar/{politico_id}/refresh-phase3")
async def refresh_phase3(campaign_id: UUID, politico_id: str, sources: Optional[dict] = None):
    """
    Phase 3: Media Intelligence (OSINT).
    
    This endpoint:
    1. Instantiates RadarCrew
    2. Runs Google/YouTube scans for the politician
    3. Returns found headlines and sentiment
    """
    from src.crew.radar_crew import RadarCrew
    
    supabase = get_supabase_client()
    
    # 0. Get Politician Details
    pol_data = supabase.table("politicians") \
        .select("name, tipo, city_id") \
        .eq("id", politico_id) \
        .single() \
        .execute()
        
    if not pol_data.data:
         raise HTTPException(status_code=404, detail="Político não encontrado")
         
    politician_name = pol_data.data.get("name")
    
    # Get City Slug
    city_slug = "brasil" # Default
    if pol_data.data.get("city_id"):
        city_res = supabase.table("cities").select("slug").eq("id", pol_data.data["city_id"]).execute()
        if city_res.data:
            city_slug = city_res.data[0]["slug"]
            
    print(f"🕵️‍♂️ Starting Phase 3 (Media) for {politician_name} in {city_slug}...")
    
    try:
        crew = RadarCrew(campaign_id=str(campaign_id))
        
        # Run scan
        # Note: run_google_scan is synchronous in current implementation
        ...
        return {
            "status": "ok",
            "message": "Varredura de mídia concluída",
            "data": media_report
        }
        
    except Exception as e:
        print(f"[Phase3] Error: {e}")
        raise HTTPException(status_code=500, detail=f"Erro na varredura de mídia: {str(e)}")
