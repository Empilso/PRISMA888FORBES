...
function createClusterIcon(pointCount: number, color: string, isCompetitor: boolean = false): L.DivIcon {
    ...
    const config = isCompetitor ? configs.red : (configs[color] || configs.gray);
    ...
    const html = `
        <div style="position: relative; width: ${size}px; height: ${size}px; display: flex; align-items: center; justify-content: center; filter: drop-shadow(0 4px 8px rgba(0,0,0,0.3));">
            <svg width="${size}" height="${size}" viewBox="0 0 100 100" style="position: absolute;">
                <defs>
                    <linearGradient id="grad-${color}-${isCompetitor}" x1="0%" y1="0%" x2="100%" y2="100%">
                        <stop offset="0%" style="stop-color:${isCompetitor ? '#EF4444' : (color === 'green' ? '#10B981' : (color === 'yellow' ? '#FBBF24' : '#991B1B'))};stop-opacity:1" />
                        <stop offset="100%" style="stop-color:${isCompetitor ? '#DC2626' : (color === 'green' ? '#059669' : (color === 'yellow' ? '#D97706' : '#7F1D1D'))};stop-opacity:1" />
                    </linearGradient>
                </defs>
                <path d="M50 5 L95 85 L5 85 Z" fill="url(#grad-${color}-${isCompetitor})" stroke="white" stroke-width="4" stroke-linejoin="round" />
            </svg>
